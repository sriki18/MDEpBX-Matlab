...
if I_strategy == 7
   F_m_mdepbx = ones(I_NP,1) * 0.5;
   Crm = ones(I_NP,1) * 0.6;
   for k1=1:I_NP
       for k2=1:I_D
        F_CR(k1,k2) = random(truncate(makedist('normal','mu',Crm(k1),'sigma',0.1),0,1));
       end
   end
end
...
FVr_ind  = zeros(4);
q = 0.15;                              % Percent of pop to select
...
while ((I_iter < I_itermax) & (S_bestval.FVr_oa(1) > F_VTR))
  ...
  elseif (I_strategy == 6)                          % either-or-algorithm
    ...
    q_inds = zeros(I_NP, floor(q*I_NP)); % q percentage indices to compare within
    for k=1:I_NP
%         Have to use setdiff and datasample to make sure that the same 
%         element is not used when calculating ui.
        q_inds(k,:) = datasample(setdiff(1:I_NP,[FVr_a1(k),FVr_a2(k)]),floor(q*I_NP),'Replace',false);
    end
    %To make Cauchy distributed F_weights specially for MDE_pBX
        % Location for Cauchy = F_m_mdepbx
        % Scale for Cauchy = 0.1
        % Must be regenerated if <0 or >1
    F_weight_mdepbx = arrayfun(@(F_m_mdepbx)trunc_cauchy(F_m_mdepbx,0.1,0,1), F_m_mdepbx);
    
    %Make candidate vector
    S_extracted = [S_val.FVr_oa];
    [~,best_col_ind] = min(S_extracted(q_inds),[],2);
    best_pop_inds = q_inds(sub2ind(size(q_inds),1:I_NP,best_col_ind'));
    FM_ui = FM_pm3 + bsxfun(@times, F_weight_mdepbx,(FM_popold(best_pop_inds,:)-FM_pm3 + FM_pm1 - FM_pm2));   
    %p-best crossover
    p = ceil(I_NP * (1 - (1-I_iter)/(I_itermax))/2 );
    S_sorted = sort(S_extracted);
    S_sorted = reshape(S_sorted,[100,1]);
    [~,p_inds] = ismember(datasample(S_sorted(1:p),I_NP),S_sorted);
    FM_ui = FM_popold(p_inds,:).*FM_mpo + FM_ui.*FM_mui;     % crossover
    FM_origin = FM_pm3;
  end
...
%----Output section----------------------------------------------------------
  ...
  % Update the control parameters for MDE_pBX (weight and crossover rate)
  if I_strategy == 7
    power_n = 1.5;
    Fsuccess = Fsuccess(Fsuccess~=0);
    power_mean_F = (sum(Fsuccess.^power_n)/length(Fsuccess)).^(1/power_n);
    wf = 0.8 + 0.2*rand;
    F_m_mdepbx = wf * F_m_mdepbx + (1-wf)*power_mean_F; 
    Fsuccess = zeros(I_NP,1);
    
    Crsuccess = Crsuccess(Crsuccess~=0);
    for k1=1:I_NP
        for k2=1:I_D
            F_CR(k1,k2) = random(truncate(makedist('normal','mu',Crm(k1),'sigma',0.1),0,1));
        end
    ...
    wcr = 0.9 + 0.1*rand;
    power_mean_cr = (sum(Crsuccess.^power_n)/length(Crsuccess)).^(1/power_n);
    Crm = wcr*Crm + (1-wcr)*power_mean_cr;
    Crsuccess = zeros(I_NP,1);
  end

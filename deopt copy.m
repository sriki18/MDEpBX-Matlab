...
%     F_weight_mdepbx = 0.1 * tan(pi*(rand(I_NP,1)-0.5)) + F_m_mdepbx;
    F_weight_mdepbx = arrayfun(@(F_m_mdepbx)trunc_cauchy(F_m_mdepbx,0.1,0,1), F_m_mdepbx);
    %Make candidate vector
    [~,best_col_ind] = min(S_val(q_inds),[],2);
    best_pop_inds = q_inds(sub2ind(size(q_inds),1:I_NP,best_col_ind));
    FM_ui = FM_pm3 + F_weight_mdepbx*(FM_popold(best_pop_inds,:)-FM_pm3 + FM_pm1 - FM_pm2);   
    %p-best crossover
    p = ceil(I_NP * (1 - (1-I_iter)/(2*I_itermax)) );
    S_sorted = sort(S_val);
    [~,p_inds] = ismember(datasample(S_sorted(1:p,I_NP)),S);
